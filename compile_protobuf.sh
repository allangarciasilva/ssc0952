...
python -m pip install grpcio-tools > /dev/null
...
python -m grpc.tools.protoc $PROTO_FILES -I $ROOT_PATH --python_out=$PY_OUT_MODULE/.. --pyi_out=$PY_OUT_MODULE/.. 2> /dev/null

echo "Building for ESP32"
python -m grpc.tools.protoc $PROTO_FILES -I $ROOT_PATH --nanopb_out=$ESP_OUTPUT_INCLUDE/.. 2> /dev/null
